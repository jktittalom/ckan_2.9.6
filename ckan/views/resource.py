...
def makeCatalogue(id):
    ...
    #print("--- Jiten Create Catalogue here ---")
    #print("-- pkg dict {}".format(pkg_dict))
    # getting 0 index value data

    if len(pkg_dict["groups"]) > 0:

        # for resource in pkg_dict['resources']:
        #     print(" resource name is: {}, {} ".format(resource['name'], resource['id']))
        ...
        # file name will be as per discussion with Joe: e.g. Counties-DeSoto-ZCTA-theme-year

        #print("current directory Path: {}".format(Path.cwd()))
        ...
        for index, resource in enumerate(pkg_dict['resources']):
            print("Resource data: {}".format(resource))
            resourceInfo = {}
            # resourceInfo['id'] = 'res_{}'.format(index)
            resourceInfo['id'] = resource['name']
            resourceInfo['name'] = resource['name']
            resourceInfo['type'] = resource['format']
            # try to use datastore csv URL to load csv from CKAN to Terria, 
            ...
            resourceInfo['defaultStyle'] = {"regionColumn":regionColumn,"color":{"numberOfBins":"5", "colorPalette":colorPalette}}

            resourceInfoaArr.append(resourceInfo)


        dataSetInfo = {}
        dataSetInfo["name"] = pkg_dict["title"]
        dataSetInfo["type"] = "group"
        dataSetInfo["description"] = pkg_dict["notes"]
        dataSetInfo["isOpen"] = True
        dataSetInfo["members"] = resourceInfoaArr


        groupInfo = {"name": pkg_dict["groups"][0]["display_name"], "type": "group", "description": pkg_dict["groups"][0]["description"],"isOpen": True, "members":[dataSetInfo]}
...
class CreateView(MethodView):
    ...
    def get(
        ...
        print("-- resource get Welcome Jiten")
